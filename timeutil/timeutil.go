...
// SetLocation set the Location with the given name.
//
// If the name is "" or "UTC", SetLocation set to UTC.
// If the name is "Local", SetLocation set to Local.
//
// Otherwise, the name is taken to be a location name corresponding to a file
// in the IANA Time Zone database, such as "America/New_York".
func SetLocation(name string) error {
	cst, err := time.LoadLocation(name)
	if err != nil {
		return err
	}
	time.Local = cst
